...
impl Migration for MigrationV0 {
    ...
    fn run(
        ...
        let name: m::ProfileName = config::DEFAULT_PROFILE_NAME.parse()?;
        let profile_id = m::Profile::create_eth_profile(
            tx_conn,
            keychain,
            &name,
            config::DEFAULT_PROFILE_PICTURE_NAME,
        )?;
...
impl MigrationV1 {
    fn default_dapp_urls() -> Vec<Url> {
        let urls = vec![
            ...
            "https://xmtp.chat/",
            "https://showtime.xyz/",
            "https://artofficialintelligence.xyz/",
        ];
        urls.iter()
...
mod tests {
    ...
    fn run_v0_migration_for_accounts(
        ...
        let profile_params = m::AccountParams::builder()
            ...
            .bundled_picture_name(config::DEFAULT_PROFILE_PICTURE_NAME)
            .build();
        #[allow(deprecated)]
        let profile_id =
